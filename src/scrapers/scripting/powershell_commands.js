      ...
      axios(url)
        .then(response => {
          try {
            ...
            const item_name = random_command.children[1].children[1].children[0].data.trim()
            const command_url = url + random_command.children[1].children[1].attribs.href
            const h2_tags =  $('h2')
            const command_category = h2_tags[0].children[0].data
            let command_description = ''
            if (random_command.children[3].children[1] !== undefined)
              command_description = random_command.children[3].children[1].children[0].data.trim()
